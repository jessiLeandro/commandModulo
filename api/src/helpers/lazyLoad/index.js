...
const isINTEGER = (type) => type instanceof Sequelize.INTEGER;

// string formartter
const isString = (type) => type instanceof Sequelize.STRING;
const assocString = (inputSearch) => {
  const searchformated = { [operators.regexp]: inputSearch };
  return searchformated;
};
...
const getAttributess = (field, model) => model.rawAttributes[field];
...
const getGlobalSearchFormated = (filter, model) => {
  ...
  const mountSearchForGlobal = R.map((field) => {
    ...
    const attribute = getAttributess(field, model);
...
const getspecificSearchFormated = (filter, model) => {
  ...
  const makeSearchEspecifField = (key) => {
    ...
    const attribute = getAttributess(key, model);
    if (!attribute) return null;
    const { type } = attribute;
...
const formatQuery = (queryPassed = null) => {
  ...
  const { attributes = null, filters = null, total = 25, page = 1 } = query;
  ...
  const getWhere = (modelName, options = {}) => {
    ...
    const filter = getfilter(JSON.parse(filters));
  ...
  const getAttributes = (modelName, options = {}) => {
    const model = database.model(modelName);

    const getattributes = R.propOr({}, modelName);

    const attributess = getattributes(JSON.parse(attributes));

    if (R.keys(attributess).length === 0) return attributes;

    return attributess.map((attribute) => {
      if (!model.rawAttributes[attribute]) return null;
      return attribute;
    });
  };

  const limitAndOffset = getLimitAndOffset(total, page);
  return { getAttributes, getWhere, ...limitAndOffset };
};
